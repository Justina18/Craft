// components/FolderPortfolio.tsx
'use client';
...
const ProjectCard = forwardRef<HTMLDivElement, ProjectCardProps>(
  ({ image, title, delay, isVisible, index, totalCount, onClick, isSelected }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={`absolute w-20 h-28 cursor-pointer group/card ${isSelected ? 'opacity-0' : ''}`}
        style={{
          transform: isVisible
      ...
      >
        <div className="w-full h-full rounded-lg overflow-hidden shadow-xl bg-gray-800 border border-white/5 relative transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover/card:-translate-y-6 group-hover/card:shadow-2xl group-hover/card:shadow-blue-500/40 group-hover/card:ring-2 group-hover/card:ring-blue-500 group-hover/card:scale-125">
          <img 
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="absolute top-6 right-6 z-50 w-12 h-12 flex items-center justify-center rounded-full bg-gray-800/30 backdrop-blur-xl border border-white/10 shadow-2xl text-white hover:bg-gray-700/30 transition-all duration-300"
        style={{
      ...
      <button
        ...
        className="absolute left-4 md:left-10 z-50 w-14 h-14 flex items-center justify-center rounded-full bg-gray-800/30 backdrop-blur-xl border border-white/10 text-white hover:scale-110 active:scale-95 transition-all duration-300 disabled:opacity-0 disabled:pointer-events-none shadow-2xl"
        style={{
      ...
      <button
        ...
        className="absolute right-4 md:right-10 z-50 w-14 h-14 flex items-center justify-center rounded-full bg-gray-800/30 backdrop-blur-xl border border-white/10 text-white hover:scale-110 active:scale-95 transition-all duration-300 disabled:opacity-0 disabled:pointer-events-none shadow-2xl"
        style={{
      ...
      >
        <div className="relative overflow-hidden rounded-[inherit] bg-gray-800 border border-white/10 shadow-[0_35px_60px_-15px_rgba(0,0,0,0.5)]">
          ...
          <div
            className="px-8 py-7 bg-gray-800 border-t border-white/5"
            style={{
              opacity: animationPhase === "complete" && !isClosing ? 1 : 0,
            ...
            <div className="flex items-center justify-between gap-6">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-4 mt-2">
                  <div className="flex items-center gap-1.5 px-2.5 py-1 bg-gray-700 rounded-full border border-white/5">
                    {projects.map((_, idx) => (
                      <button
                        key={idx}
                        onClick={() => handleDotClick(idx)}
                        className={`w-1.5 h-1.5 rounded-full transition-all duration-500 ${idx === internalIndex ? "bg-white scale-150" : "bg-gray-500 hover:bg-gray-400"}`}
                      />
                    ))}
                  </div>
              ...
              <button className="flex items-center gap-2 px-6 py-3 text-sm font-bold uppercase tracking-widest text-white bg-blue-600 hover:brightness-110 rounded-xl shadow-lg shadow-blue-600/20 transition-all duration-300 hover:scale-105 active:scale-95">
...
const AnimatedFolder: React.FC<AnimatedFolderProps> = ({ title, projects, gradient }) => {
  ...
  const backBg = gradient || "linear-gradient(135deg, #374151 0%, #1f2937 100%)";
  const tabBg = gradient || "#1f2937";
  const frontBg = gradient || "linear-gradient(135deg, #4b5563 0%, #374151 100%)";

  return (
    <>
      <div
        className="relative flex flex-col items-center justify-center p-8 rounded-2xl cursor-pointer bg-gray-800 border border-gray-700 transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] hover:shadow-2xl hover:shadow-blue-500/20 hover:border-blue-500/40 group"
      ...
      >
        <div
          ...
          style={{ background: gradient ? `radial-gradient(circle at 50% 70%, ${gradient.match(/#[a-fA-F0-9]{3,6}/)?.[0] || '#3b82f6'} 0%, transparent 70%)` : "radial-gradient(circle at 50% 70%, #3b82f6 0%, transparent 70%)", opacity: isHovered ? 0.12 : 0 }}
        />
        ...
        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-widest text-gray-500 transition-all duration-500" style={{ opacity: isHovered ? 0 : 1, transform: isHovered ? "translateY(10px)" : "translateY(0)" }}>
          <span>Hover</span>
        </div>
      </div>
